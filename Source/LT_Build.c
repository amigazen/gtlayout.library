...
LT_BuildA(REG(a0) LayoutHandle *handle,REG(a1) struct TagItem *TagParams)
{
	...
	if(handle->Window = OpenWindowTags(NULL,
		...
		TAG_MORE,Tags))
			...
			This is NOT possible as all gtlayout tags starting with (TAG_USER+99) clash with
			Intuition tags which start at the very same value. Handing our tags over to
			intuition *will* in 99.9% crash the machine!
			Hence, we have no other choice than to dismiss those tags.
		*/
        TAG_DONE))
	{
